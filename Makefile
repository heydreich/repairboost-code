...
CC_FILES = $(filter-out $(SRC_DIR)/ECCoordinator.cc $(SRC_DIR)/ECHelper.cc  $(SRC_DIR)/ECClient.cc $(SRC_DIR)/Simulation.cc, \
			$(wildcard $(SRC_DIR)/*.cc))
...
all : directories ECCoordinator ECHelper  ECClient Simulation
...
Simulation : $(SRC_DIR)/Simulation.cc $(O_FILES)
	$(CC) $(CFLAGS) -o $@ $^ $(CLIBS)
